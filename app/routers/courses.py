...
@router.post("", response_model=schemas.CourseOut, status_code=201, summary="Create a course")
def create_course(data: schemas.CourseCreate, db: Session = Depends(get_db)):
    return crud.create_course(db, data)


@router.get("", response_model=list[schemas.CourseOut], summary="List all courses")
def list_courses(db: Session = Depends(get_db)):
    return crud.get_courses(db)


@router.get("/{course_id}", response_model=schemas.CourseOut, summary="Get course by ID")
...
@router.put("/{course_id}", response_model=schemas.CourseOut, summary="Update course")
