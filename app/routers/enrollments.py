...
@router.post("", response_model=schemas.EnrollmentOut, status_code=201, summary="Create an enrollment")
...
@router.get("", response_model=list[schemas.EnrollmentOut], summary="List all enrollments")
def list_enrollments(db: Session = Depends(get_db)):
    return crud.get_enrollments(db)


@router.get("/{enrollment_id}", response_model=schemas.EnrollmentOut, summary="Get enrollment by ID")
...
@router.delete("/{enrollment_id}", summary="Delete enrollment")
